import 'package:flutter/material.dart';
import 'app_colors.dart';

class ComponentThemes {
  // AppBar Theme
  static AppBarTheme lightAppBarTheme = const AppBarTheme(
    backgroundColor: AppColor.lightPrimary,
    foregroundColor: AppColor.lightOnPrimary,
    elevation: 2,
    centerTitle: true,
    titleTextStyle: TextStyle(
      fontSize: 20,
      fontWeight: FontWeight.w600,
      color: AppColor.lightOnPrimary,
    ),
    iconTheme: IconThemeData(color: AppColor.lightOnPrimary),
  );

  static AppBarTheme darkAppBarTheme = const AppBarTheme(
    backgroundColor: AppColor.darkSurface,
    foregroundColor: AppColor.darkOnPrimary,
    elevation: 2,
    centerTitle: true,
    titleTextStyle: TextStyle(
      fontSize: 20,
      fontWeight: FontWeight.w600,
      color: AppColor.darkTextPrimary,
    ),
    iconTheme: IconThemeData(color: AppColor.darkTextPrimary),
  );

  // Elevated Button Theme
  static ElevatedButtonThemeData lightElevatedButtonTheme =
      ElevatedButtonThemeData(
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColor.lightPrimary,
          foregroundColor: AppColor.lightOnPrimary,
          elevation: 2,
          padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 12),
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(12),
          ),
          textStyle: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600),
        ),
      );

  static ElevatedButtonThemeData darkElevatedButtonTheme =
      ElevatedButtonThemeData(
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColor.darkPrimary,
          foregroundColor: AppColor.darkOnPrimary,
          elevation: 2,
          padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 12),
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(12),
          ),
          textStyle: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600),
        ),
      );

  // Input Decoration Theme
  static InputDecorationTheme lightInputDecorationTheme = InputDecorationTheme(
    border: OutlineInputBorder(
      borderRadius: BorderRadius.circular(12),
      borderSide: const BorderSide(color: Colors.grey),
    ),
    enabledBorder: OutlineInputBorder(
      borderRadius: BorderRadius.circular(12),
      borderSide: const BorderSide(color: Colors.grey),
    ),
    focusedBorder: OutlineInputBorder(
      borderRadius: BorderRadius.circular(12),
      borderSide: const BorderSide(color: AppColor.lightPrimary, width: 2),
    ),
    errorBorder: OutlineInputBorder(
      borderRadius: BorderRadius.circular(12),
      borderSide: const BorderSide(color: AppColor.lightError, width: 2),
    ),
    focusedErrorBorder: OutlineInputBorder(
      borderRadius: BorderRadius.circular(12),
      borderSide: const BorderSide(color: AppColor.lightError, width: 2),
    ),
    labelStyle: const TextStyle(color: AppColor.lightTextSecondary),
    hintStyle: const TextStyle(color: AppColor.lightTextSecondary),
    contentPadding: const EdgeInsets.all(16),
  );

  static InputDecorationTheme darkInputDecorationTheme = InputDecorationTheme(
    border: OutlineInputBorder(
      borderRadius: BorderRadius.circular(12),
      borderSide: const BorderSide(color: Colors.grey),
    ),
    enabledBorder: OutlineInputBorder(
      borderRadius: BorderRadius.circular(12),
      borderSide: const BorderSide(color: Colors.grey),
    ),
    focusedBorder: OutlineInputBorder(
      borderRadius: BorderRadius.circular(12),
      borderSide: const BorderSide(color: AppColor.darkPrimary, width: 2),
    ),
    errorBorder: OutlineInputBorder(
      borderRadius: BorderRadius.circular(12),
      borderSide: const BorderSide(color: AppColor.darkError, width: 2),
    ),
    focusedErrorBorder: OutlineInputBorder(
      borderRadius: BorderRadius.circular(12),
      borderSide: const BorderSide(color: AppColor.darkError, width: 2),
    ),
    labelStyle: const TextStyle(color: AppColor.darkTextSecondary),
    hintStyle: const TextStyle(color: AppColor.darkTextSecondary),
    contentPadding: const EdgeInsets.all(16),
  );

  // Card Theme
  static CardThemeData lightCardTheme = CardThemeData(
    elevation: 4,
    color: AppColor.lightSurface,
    shadowColor: Colors.black.withValues(alpha: 0.1),
    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
    margin: const EdgeInsets.all(8),
  );

  static CardThemeData darkCardTheme = CardThemeData(
    elevation: 4,
    color: AppColor.darkSurface,
    shadowColor: Colors.black.withValues(alpha: 0.3),
    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
    margin: const EdgeInsets.all(8),
  );

  // Bottom Navigation Bar Theme
  static BottomNavigationBarThemeData lightBottomNavTheme =
      const BottomNavigationBarThemeData(
        backgroundColor: AppColor.lightSurface,
        selectedItemColor: AppColor.lightPrimary,
        unselectedItemColor: AppColor.lightTextSecondary,
        type: BottomNavigationBarType.fixed,
        elevation: 8,
      );

  static BottomNavigationBarThemeData darkBottomNavTheme =
      const BottomNavigationBarThemeData(
        backgroundColor: AppColor.darkSurface,
        selectedItemColor: AppColor.darkPrimary,
        unselectedItemColor: AppColor.darkTextSecondary,
        type: BottomNavigationBarType.fixed,
        elevation: 8,
      );
}
